...
three months ago, and when did that change*. That question is most of the reason to
...
- **`belief` moves on evidence, not on age.** A claim reinforced by a new session should
  ...
  nobody has touched is not thereby false, it is merely unexamined. That is what
...
- duplicate basenames, which make every existing link to the original silently ambiguous
